...
class RoomLogic:
    ...
    def insert_room(self, **kwargs):
        return Room.objects.create(**kwargs)

    @classmethod
    def calculate_price_based_on_regular_price(cls, regular_price):
        highest_price = 2 * regular_price
        lowest_price = 0.75 * regular_price
        middle_price = 1.25 * regular_price
    ...
    def calculate_and_add_seats(self, user_id, room_length, regular_seat_price, room_type) -> str:
        ...
        seat_info, rows_of_room = self.seat_logic.calculate_seat_price(length=room_length)

        room = self.insert_room(count_of_rows=rows_of_room, count_of_seats=rows_of_room ** 2,
                                business_owner=business_owner, length=room_length, type=RoomType(room_type))

        highest_price, lowest_price, middle_price = self.calculate_price_based_on_regular_price \
            (regular_price=regular_seat_price)
        ...
        return f"Your room with {sum(seat_info.values())} seats added successfully"